 ...
 *                  §6.4.8 core). SELF-ACTUALIZING: driver.move() throws.
 *   RFC6851-3.3-2  UID MOVE command form: UID SP MOVE SP sequence-set SP mailbox.
 *                  profiles ["rev1"] (rev2 via core). SELF-ACTUALIZING:
 *                  driver.uidMove() throws.
 ...
 *                  residue). SELF-ACTUALIZING: no MOVE surface (move() throws).
 ...
 *                  SELF-ACTUALIZING: no MOVE surface.
 ...
 *                  extension in rev2). SELF-ACTUALIZING: no UID MOVE surface.
...
// ═════════════════════════════════════════════════════════════════════════════
// RFC6851-3.3-1 — MOVE SP sequence-set SP mailbox (SELF-ACTUALIZING, rev1-only)
// ═════════════════════════════════════════════════════════════════════════════
// §5 ABNF: move = "MOVE" SP sequence-set SP mailbox. driver.move() throws today →
// unimplemented. The matcher pins the atom MOVE, a sequence-set, and a mailbox
// name so a wrong impl (e.g. one that emits COPY+STORE+EXPUNGE separately, or a
// MOVE missing its mailbox argument) is rejected once implemented.
complianceTest(
	{
		reqs: ["RFC6851-3.3-1"],
		profiles: ["rev1"],
	...
	async () => {
		...
		await driver.move("3:5", "Archive"); // throws NotImplementedError today
		await server.assertCompleted();
		const move = server.commandLines.find((l) => l.verb === "MOVE");
		expect(move, "MOVE must have been emitted").toBeDefined();
		// When implemented: a sequence-set precedes the mailbox — never a bare MOVE.
...
// ═════════════════════════════════════════════════════════════════════════════
// RFC6851-3.3-2 — UID SP MOVE ... (SELF-ACTUALIZING, rev1-only)
// ═════════════════════════════════════════════════════════════════════════════
...
// sequence-set interpreted as UIDs. driver.uidMove() throws → unimplemented. The
// two-token 'UID MOVE' verb rejects a bare 'MOVE' (missing the UID prefix, which
// would make the server treat the argument as a sequence number).
complianceTest(
	{
		reqs: ["RFC6851-3.3-2"],
		profiles: ["rev1"],
	...
	async () => {
		...
		const driver = await f.connectPlain(server);
		await driver.uidMove("42:69", "foo"); // throws NotImplementedError today
		await server.assertCompleted();
...
// ═════════════════════════════════════════════════════════════════════════════
// RFC6851-3.3-3 — accept COPY+EXPUNGE, no STORE code, no \Deleted (SELF-ACTUAL.)
// ═════════════════════════════════════════════════════════════════════════════
// A successful MOVE reply carries the COPY-related code(s) and EXPUNGE responses
// but NO STORE response code and sets no \Deleted. driver.move() throws today →
// unimplemented. The scripted reply deliberately includes COPYUID + EXPUNGE and
// deliberately OMITS any STORE/FETCH \Deleted response; the transcript guard is a
// second layer confirming the client never demanded a STORE round-trip.
complianceTest(
	{
		reqs: ["RFC6851-3.3-3"],
		profiles: ["rev1", "rev2"],
	...
	async () => {
		...
		const move = server.commandLines.find((l) => l.verb === "MOVE");
		expect(move, "MOVE must have been emitted").toBeDefined();
		// When implemented: the client completes the MOVE without ever issuing a
		// STORE (the workaround-sequence command it must NOT require here).
		expect(
...
// ═════════════════════════════════════════════════════════════════════════════
// RFC6851-3.3-4 — no seq-number commands mid-MOVE (SELF-ACTUALIZING, rev1-only)
// ═════════════════════════════════════════════════════════════════════════════
...
// driver.move() throws → unimplemented. The script accepts the MOVE + its EXPUNGE
// responses but scripts NO interleaved seq-number command; only a NOOP (no
// seq-number argument) follows. The transcript guard independently rejects any
// FETCH/STORE/SEARCH interleaved after MOVE.
complianceTest(
	{
		reqs: ["RFC6851-3.3-4"],
		profiles: ["rev1"],
	...
	async () => {
		...
		server.arm([
			[
				...
				// after MOVE would be an unscripted-command failure once the verb lands.
				expectLine(command("MOVE")),
				reply("OK MOVE completed", ["* OK [COPYUID 1 2 2] Moved", "* 2 EXPUNGE"]),
			],
		]);
		const driver = await f.connectPlain(server);
		await driver.move("2", "Archive"); // throws NotImplementedError today
		await server.assertCompleted();
		const move = server.commandLines.find((l) => l.verb === "MOVE");
		expect(move, "MOVE must have been emitted").toBeDefined();
...
// ═════════════════════════════════════════════════════════════════════════════
// RFC6851-4.4-1 — QRESYNC client handles VANISHED and EXPUNGE (SELF-ACTUALIZING)
// ═════════════════════════════════════════════════════════════════════════════
// A QRESYNC-enabled client must process both VANISHED and EXPUNGE responses to a
// UID MOVE. driver.uidMove() throws → unimplemented. The scripted UID MOVE reply
// carries a VANISHED response; once a UID MOVE surface exists the client must
// consume it. profiles ["rev1","rev2"] (QRESYNC is a separate extension in rev2,
// no §6.4.8 restatement of this handling duty).
complianceTest(
	{
		reqs: ["RFC6851-4.4-1"],
		profiles: ["rev1", "rev2"],
	...
	async () => {
		...
		server.arm([
			[
				...sessionPrelude(["IMAP4rev1", "MOVE", "QRESYNC"]),
