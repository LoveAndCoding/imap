 ...
 * against the advertised-capability gate, so once an AUTHENTICATE surface exists
 * the matchers ARE the genuine assertions.
 */
...
// never quoted or a literal announcement. authenticate() throws today.
complianceTest(
	{
	...
	async () => {
		...
		// When implemented: IR argument is bare base64, outside quotes/literal.
...
// with the response arriving on a continuation line. authenticate() throws today.
complianceTest(
	{
		reqs: ["RFC4959-3-3"],
		profiles: ["rev1"],
	...
	async () => {
		...
		const authLine = server.commandLines.find((l) => l.verb === "AUTHENTICATE");
		expect(authLine).toBeDefined();
		// When implemented: no IR argument may appear when SASL-IR is unadvertised.
