...
export const note =
	...
	"§5.1 Mailbox Naming — extracted 6 (rev2 replaces RFC3501's mod-UTF-7 encoding regime with direct Net-Unicode/UTF-8 mailbox names — §5.1.3 'Mailbox International Naming Convention' and all its modified-BASE64 encoding rules are gone entirely; 1 MAY/MUST compound on creating/interpreting Net-Unicode names, 1 judgment-MUST NOT elision-note on the server-prohibition half of that same sentence being server-only, 1 judgment-testable on the case-sensitivity 'takes no position' framing, 1 untestable-MUST carried over unchanged from RFC3501-5.1-3 on interacting with any case model, 1 testable-MUST on atom-specials quoting, 1 judgment-testable on the '#'/'&' conventional-meaning advisory item — item 5 of the client-considerations list, promoted to testable here because RFC 9051 keeps the lowercase lead-in 'certain client considerations' but the pre-9051 catalog treated the identical list as non-normative advisory prose; re-examined in place, see item 5 rationale below). " +
	...
	"Total entries: 14. Untestable: 1 (RFC9051-5.1-4, carried over from RFC3501-5.1-3, theme internal-decision).";
...
export const requirements: SpecRequirement[] = [
	...
	},
	{
		...
		source: "RFC9051",
		section: "5.1",
		title: "Specification takes no position on case sensitivity of non-INBOX mailbox names",
		text: "In particular, this specification takes no position on case sensitivity in non-INBOX mailbox names.",
		level: "MUST",
		applicability: "always",
		profiles: ["rev2"],
		...
		untestableRationale:
			"This sentence is descriptive framing (no RFC 2119 keyword) introducing the three server case-sensitivity models enumerated in the next sentence; it states a design fact about the specification, not a directly testable client action by itself. It is catalogued here (rather than folded silently into RFC9051-5.1-4) because it is the antecedent-setting sentence for that entry's 'any of these', and separating them preserves verbatim-quotation fidelity for both. No independent client behavior can be observed against this sentence alone.",
		notes:
			"Companion/antecedent sentence to RFC9051-5.1-4 (the actual client MUST). Carried forward with identical substance from RFC3501-5.1 (untitled paragraph, not separately id'd in the RFC3501 catalog — this entry closes that gap for rev2 since the sentence is quoted as the direct antecedent of the client MUST in the following sentence).",
	},
	{
		id: "RFC9051-5.1-4",
		source: "RFC9051",
		...
		text: "Client implementations must be able to interact with any of these.",
		level: "MUST",
		applicability: "always",
		profiles: ["rev2"],
		testability: "untestable",
		untestableTheme: "internal-decision",
		untestableRationale:
			"Direct carry-forward of RFC3501-5.1-3's untestability analysis: this is a blanket obligation to interoperate with all three server case-sensitivity models (fully case-sensitive in ASCII range, case-insensitive but case-preserving, case-insensitive with case-coercion) described in the immediately preceding sentence. There is no single observable protocol interaction that proves general compliance; a client can pass any particular test case yet still fail with an untested server variant. The harness could script all three server models, but no single client wire behavior would distinguish universal compliance — mailbox names originate with the consuming application and the library forwards them, so the 'interacts with any model' property has no finite black-box observable. Full coverage would require an open-ended integration property, not a single assertion.",
		notes:
			"The keyword 'must' is lowercase in the RFC 9051 text (identical wording and casing to RFC3501-5.1-3's 'MUST', but RFC 9051 prints it lowercase here — pre-RFC-8174 convention does not formally distinguish case, and RFC 9051 is itself an RFC-8174-era document, so this lowercase instance is flagged as a judgment call). Level assigned as MUST by judgment, consistent with the RFC3501 predecessor's explicit uppercase MUST for the same substantive duty. The antecedent 'any of these' refers to the three case-sensitivity models named in the preceding sentence (fully case-sensitive; case-insensitive with case-preservation; case-insensitive with case-coercion). Practical implication: clients must never assume a particular server model and must reuse exact server-returned mailbox name strings in subsequent commands.",
	},
	{
		id: "RFC9051-5.1-5",
		source: "RFC9051",
