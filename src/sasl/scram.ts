 ...
 * whole exchange (including the tagged status) has played out. A missing
 * server-final-message entirely (RFC5802-5.1-14: legal on failure, and also
 * what a server that only signals success via the tagged OK, without ever
 * sending "v=", produces) leaves nothing to verify — `finish()` has no
 * verdict to render in that case and resolves normally; it is `step()`'s
 * *first*-message handling (nonce-prefix mismatch, an unsupported mandatory
 * "m=" extension) that aborts synchronously via a thrown error, since those
...
export interface ScramMechanismOptions {
	 ...
	 *  authentication") — a mechanism instance is already fresh-per-attempt
	 *  (`createMechanism()`'s documented contract), so the only thing this
	 *  seam exists for is pinning a KNOWN nonce to reproduce an RFC worked
