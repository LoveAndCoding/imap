...
export interface ListCapabilityProbe {
	 ...
	 *  capability forms (e.g. APPENDLIMIT=<n>); absent means "none". */
...
export class ListCommand extends Command<MailboxInfo[]> {
	...
	constructor(opts: ListOptions = {}, caps: ListCapabilityProbe = NO_CAPS) {
		...
		if (statusItems.length > 0) {
			// Per-item gates (SIZE -> STATUS=SIZE|IMAP4rev2, APPENDLIMIT,
			// MAILBOXID -> OBJECTID, HIGHESTMODSEQ -> CONDSTORE|QRESYNC, ...):
			// one shared rule with the standalone STATUS command so LIST's
			// RETURN (STATUS ...) can never request an item STATUS itself
			// would refuse (spec I-9; helper owned by commands/status.ts).
			assertStatusItemsSupported(statusItems, {
				has: (cap) => caps.has(cap),
				all: () => caps.all?.() ?? new Set<string>(),
			});
		}
		if (
